            ...
            items: [
                {
                    ...
                    items: [
                        ...
                        { boxLabel: 'Actor names', name: 'sets', checked: true, checked: true, inputValue: 'Actors'},
                        { boxLabel: 'Writer names', name: 'sets', checked: true, checked: true, inputValue: 'Writers' },
                        { boxLabel: 'Production team members', name: 'sets', checked: false, inputValue: 'Production' },
                        { boxLabel: 'Cards Against Humanity', name: 'sets', checked: false, inputValue: 'CardsAgainstHumanity' },
                        { boxLabel: 'First Expansion', name: 'sets', checked: false, inputValue: 'CAHe1'},
                        { boxLabel: 'Second Expansion', name: 'sets', checked: false, inputValue: 'CAHe2' },
...
function doReaderTurn() {
    ...
    if (reader.id == user.id) {
        ...
        function checkSubmission()
        {
            ...
            Ext.getCmp('gameStatePanel').setTitle("Round Timer:" + (110-numLoops) + " seconds");
            if (numSubmissions == numPlayers-1 || numLoops > 110) {
                clearInterval(sLoop);
...
function setReader(eventData) {
    ...
    gapi.hangout.layout.displayNotice(reader.name + " is the giant hipster in the room!");
...
function winnerPicked(eventData) {
    ...
    if (points>= winningPoints) {
        ...
        readerVideoWindow.setTitle('Soulless, joyless newsie: ' + eventData.playerName);
        ...
        if (user.id == eventData.playerID) {
            var pancakes = gapi.hangout.av.effects.createImageResource('https://dl.dropbox.com/u/44863/LBD/img/winner_pancake.png');
        ...
        var winnerFeed = gapi.hangout.layout.createParticipantVideoFeed(playerRecord.get('participantID'));
        videoCanvas.setVideoFeed(winnerFeed);
        readerVideoWindow.setTitle('Agoraphobic lobster cam: ' + eventData.playerName);
